...
def create_combined_stimulus_trace(behaviour_matrix, trace_name_list):
    ...
    ai_channel_dict = Transition_Utils.create_stimuli_dictionary()
...
def regression_motor_correction(trial_activity, trial_start, trial_stop, behaviour_matrix, regression_dictionary):

    # Get Lick and Running Traces
    stimuli_dictionary = Transition_Utils.create_stimuli_dictionary()
    ...
    design_matrix = np.vstack([
        lick_trace[trial_start:trial_stop],
        running_trace[trial_start:trial_stop]
    ])
...
def get_ragged_tensors(activity_matrix, full_behaviour_matrix, selected_ai_channels, trial_start_stop_list, movement_correction, regression_dict=None):

    # Get Selected Traces
    channel_index_dictionary = Transition_Utils.create_stimuli_dictionary()
    ...
    for trial_index in range(number_of_trials):
        ...
        if movement_correction == True:
            trial_activity = regression_motor_correction(trial_activity, trial_start, trial_stop, full_behaviour_matrix, regression_dict)
...
def create_extended_standard_alignment_tensor(base_directory, tensor_save_directory, onsets_file, start_window, stop_stimuli, selected_ai_channels, trace_threshold=0.5, start_cutoff=3000, min_trial_number=1, max_trial_num=50, movement_correction=True):
    ...
    if len(onsets_list) < min_trial_number:
        ...
        # Get Ragged Tensors
        activity_tensor, behaviour_tensor = get_ragged_tensors(activity_matrix, downsampled_ai_matrix, selected_ai_channels, trial_start_stop_list, movement_correction, regression_dict=regression_dictionary)
