...
def align_activity_tensor(base_directory, activity_tensor):
    ...
    consensus_indicies, consensus_image_height, consensus_image_width = load_tight_mask()
    print("Consensus height", consensus_image_height)
    print("Consensus Width", consensus_image_width)
    ...
    for frame in activity_tensor:
        ...
        # Reconstruct Image
        frame = create_image_from_data(frame, indicies, image_height, image_width)
        #plt.imshow(frame)
        #plt.title("pre alignmnet")
        #plt.show()

        # Align Image
        frame = transform_image(frame, alignment_dictionary, invert=False)
        #plt.imshow(frame)
        #plt.title("post alignmnet")
        #plt.show()
        ...
        """
        template = np.zeros(image_height * image_width)
        template[consensus_indicies] = frame
...
def load_ai_data(base_directory):

    ai_file_name = get_ai_filename(base_directory)
    ai_recorder_file_location = os.path.join(base_directory, ai_file_name)
    table = tables.open_file(ai_recorder_file_location, mode='r')
    data = table.root.Data
