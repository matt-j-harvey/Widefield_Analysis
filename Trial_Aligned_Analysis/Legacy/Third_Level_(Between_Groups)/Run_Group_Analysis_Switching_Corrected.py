...
def get_single_group_activity_tensor_list(session_list, tensor_names, tensor_save_directory, paired_or_unpaired):
    ...
    for condition_index in range(number_of_conditions):
        ...
        for base_directory in session_list:
            ...
            # Load Activity Tensor
            activity_tensor = np.load(os.path.join(tensor_save_directory, mouse_name, session_name, condition_name + "_Residual_Tensor.npy"), allow_pickle=True)
            print("Activity Tensor Shape", np.shape(activity_tensor))
            # Get Average
            mean_activity = np.nanmean(activity_tensor, axis=0)
...
save_directory = r"/media/matthew/Expansion/Thesis_Comittee_Analysis/Uncorrected_Switching_Modulation/Mutants_Residual"
...
save_directory = r"/media/matthew/Expansion/Thesis_Comittee_Analysis/Uncorrected_Switching_Modulation/Controls_Residual"
