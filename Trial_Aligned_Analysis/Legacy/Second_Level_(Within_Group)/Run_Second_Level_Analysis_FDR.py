...
def paired_significance_fdr(activity_tensor, save_directory):
    ...
    # Convert To Diffference Tensor For 1 Sample T Test
    print("Activity Tensor Shape", np.shape(activity_tensor))
    number_of_mice = np.shape(activity_tensor)[1]
    number_of_timepoints = np.shape(activity_tensor)[2]

    print("Number Of Timepoints", number_of_timepoints)

    for timepoint_index in range(number_of_timepoints):

        condition_1_data = activity_tensor[0, :, timepoint_index]
        condition_2_data = activity_tensor[1, :, timepoint_index]
        print("Condition 1 data", np.shape(condition_1_data))


        # Flatten Data
        condition_1_data = np.reshape(condition_1_data, (number_of_mice, image_height * image_width))
        condition_2_data = np.reshape(condition_2_data, (number_of_mice, image_height * image_width))
        print("Condition 1 data", np.shape(condition_1_data))


        # Take Mask
        condition_1_data = condition_1_data[:, indicies]
        condition_2_data = condition_2_data[:, indicies]
        print("Condition 1 data", np.shape(condition_1_data))

        # Needs To Be Sqeezed - Adds an Extra Dimension For Some Reaseon
        condition_1_data = np.ndarray.squeeze(condition_1_data, axis=1)
        condition_2_data = np.ndarray.squeeze(condition_2_data, axis=1)
        print("Condition 1 data", np.shape(condition_1_data))
        ...
        # Perform FDR
        rejected, p_values = statsmodels.stats.multitest.fdrcorrection(p_values)

        # Get Inverse Ps
        inverse_p = 1 - p_values

        template = np.zeros(image_height * image_width)
        template[indicies] = inverse_p
        template = np.reshape(template, (image_height, image_width))

        plt.title(str((timepoint_index-10)*36))
        plt.imshow(template, vmin=0.95, vmax=1)
        plt.colorbar()
        plt.axis('off')
...
def paired_significance_fdr_window(activity_tensor, save_directory):
    ...
    condition_1_data = activity_tensor[0, :, window_start:window_stop]
    condition_2_data = activity_tensor[1, :, window_start:window_stop]
    print("Condition 1 data", np.shape(condition_1_data))

    condition_1_data = np.mean(condition_1_data, axis=1)
    condition_2_data = np.mean(condition_2_data, axis=1)
    print("Condition 1 data", np.shape(condition_1_data))
    ...
    plt.imshow(template, vmin=0.95) #vmax=1 #vmin=0.95
    plt.colorbar()
    plt.axis('off')
    plt.show()
...
#paired_significance_fdr(mean_mouse_tensor, save_directory)

save_directory = r"/home/matthew/Documents/Adil_Meeting_2022_12_01/Control_Contextual_Modulation/n_session/Window"
mean_session_tensor = np.load(os.path.join(signficance_testing_folder, "Controls_Contextual_Modulation_Session_Average.npy"))
#paired_significance_fdr(mean_session_tensor, save_directory)
paired_significance_fdr_window(mean_session_tensor, save_directory)

#save_directory = r"/home/matthew/Documents/Adil_Meeting_2022_12_01/Control_Contextual_Modulation/n_mouse/Window"
#paired_significance_fdr_window(mean_mouse_tensor, save_directory)