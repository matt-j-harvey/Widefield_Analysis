...
def mixed_effects_random_slope_and_intercept(dataframe):
    ...
    print("Pixel Parameters", parameters)
...
def test_significance_window_session_average(tensor_directory, analysis_name):
    ...
    for session in unique_sessions:
        ...
        print("Session indicies: ", session_indicies)
        ...
        condition_1_mean = np.mean(condition_1_mean[14:28], axis=0)
        condition_2_mean = np.mean(condition_2_mean[14:28], axis=0)

        modulation = np.subtract(condition_1_mean, condition_2_mean)
        modulation_list.append(modulation)
        print("Session trials length", np.shape(session_trials))


    inidices, image_height, image_width = widefield_utils.load_tight_mask()
    inidices, image_height, image_width = widefield_utils.downsample_mask_further(inidices, image_height, image_width)

    figure_1 = plt.figure()
    number_of_sessions = len(modulation_list)
    for session_index in range(number_of_sessions):
        axis = figure_1.add_subplot(1,number_of_sessions, session_index + 1)
        image = widefield_utils.create_image_from_data(modulation_list[session_index],  inidices, image_height, image_width)

        axis.imshow(image, cmap=widefield_utils.get_musall_cmap(), vmin=-0.01, vmax=0.01)
