...
def view_average_difference(tensor_directory, condition_1_index, condition_2_index, comparison_name, start_window, stop_window,  vmin=-0.05, vmax=0.05, diff_magnitude=0.5):
    ...
    for timepoint_index in tqdm(range(number_of_timepoints), position=0, desc="Timepoint"):
        ...
        # Add Colourbars
        condition_1_colourbar = figure_1.colorbar(cm.ScalarMappable(norm=Normalize(vmin=vmin * 100, vmax=vmax * 100), cmap=delta_f_cmap), ax=condition_1_axis, fraction=0.046, pad=0.04)
        condition_2_colourbar = figure_1.colorbar(cm.ScalarMappable(norm=Normalize(vmin=vmin * 100, vmax=vmax * 100), cmap=delta_f_cmap), ax=condition_2_axis, fraction=0.046, pad=0.04)
        diff_colourbar = figure_1.colorbar(cm.ScalarMappable(norm=Normalize(vmin=diff_v_min * 100, vmax=diff_v_max * 100), cmap=difference_cmap), ax=diff_axis, fraction=0.046, pad=0.04)
...
def view_average_difference_per_mouse(tensor_directory, condition_1_index, condition_2_index, comparison_name, start_window, stop_window, vmin=-0.05, vmax=0.05, diff_scale_factor=0.5):
    ...
    # Plot This
    for timepoint_index in tqdm(range(number_of_timepoints), position=0, desc="Timepoint"):
        figure_1 = plt.figure(figsize=[0.1, 5])
        ...
        for mouse_index in range(n_mice):
            ...
            # Resize
            condition_1_image = resize(condition_1_image, (600, 600), anti_aliasing=True)
            condition_2_image = resize(condition_2_image, (600, 600), anti_aliasing=True)
            difference_image = resize(difference_image, (600, 600), anti_aliasing=True)

            # Smooh Outline
            condition_1_image[edge_indicies] = [0, 1, 0, 1]
            condition_2_image[edge_indicies] = [0, 1, 0, 1]
            difference_image[edge_indicies] = [0, 1, 0, 1]
