...
def plot_roi_trace_average(condition_1_data, condition_2_data, roi_list, roi_name, start_window, stop_window, save_directory):
    ...
    scaled_significance = np.multiply(significance, max_value)
    print("P Values", p_values)

    x_values = list(range(int(start_window), int(stop_window)))
    x_values = np.multiply(x_values, 36)

    plt.plot(x_values, condition_1_mean, color='cornflowerblue', linestyle='dashed')
...
def quantify_roi_activity_across_conditions(tensor_directory, condition_1_index, condition_2_index, roi_list, roi_name, start_window, stop_window):
    ...
    # Load Data
    condition_averages = np.load(os.path.join(tensor_directory, "Average_Activity", "Mouse_Condition_Average_Matrix.npy"))
