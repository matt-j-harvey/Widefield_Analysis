...
def create_analysis_dataset(tensor_directory_list, output_directory, nested_session_list, condition_list, analysis_name, start_window, stop_window):
    ...
    # Open File
    analysis_dataset_file = tables.open_file(filename=os.path.join(output_directory, analysis_name + "_Trialwise_.h5"), mode="w")
    ...
    for group in nested_session_list:
        for mouse in group:
            for session in mouse:
                ...
                # Load Regressor Dict
                full_tensor_directory = os.path.join(tensor_directory_list[group_index], session)
