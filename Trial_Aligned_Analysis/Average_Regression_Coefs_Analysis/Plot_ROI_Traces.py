...
def plot_roi_trace_average(condition_1_data, condition_2_data, roi_list, roi_name, start_window, stop_window, save_directory):
    ...
    plt.plot(x_values, condition_1_mean, color='cornflowerblue')
    plt.plot(x_values, condition_2_mean, color='orange')

    plt.fill_between(x=x_values, y1=c1_lower_bound, y2=c1_upper_bound, alpha=0.3, color='cornflowerblue')
    plt.fill_between(x=x_values, y1=c2_lower_bound, y2=c2_upper_bound, alpha=0.3, color='orange')
...
def plot_individual_traces(condition_1_data, condition_2_data, roi_list, roi_name, start_window, stop_window):
    ...
    pooled_roi_pixels = []
    for roi in roi_list:
        roi_pixel_indicies = ROI_Quantification_Functions.get_roi_pixels(roi)
        for index in roi_pixel_indicies:
            pooled_roi_pixels.append(index)

    print("ROI Pixels Indicies", np.shape(roi_pixel_indicies))
...
def quantify_roi_activity_across_conditions_individuals(tensor_directory, condition_1_index, condition_2_index, baseline_window, roi_list, roi_name, start_window, stop_window):
    """
        This Test Is Run Pixelwise - All Brains Must Be In Same Pixel Space
        """
