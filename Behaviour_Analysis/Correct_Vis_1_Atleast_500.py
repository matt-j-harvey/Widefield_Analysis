...
def visualise_traces(base_directory):
    ...
    # Load Onsets
    onsets_list = np.load(os.path.join(base_directory, "Stimuli_Onsets", "Correct_Vis_1_atleast_500.npy"))
...
def get_lick_aligned_hits(base_directory, early_cutoff=3000):
    ...
    for trial in behaviour_matrix:
        ...
        if trial_type == 1 and correct == 1 and reaction_time > 500:
            ...
            if nearest_frame > early_cutoff:
                lick_aligned_hits.append(nearest_frame)

    np.save(os.path.join(base_directory,  "Stimuli_Onsets", "Correct_Vis_1_atleast_500.npy"), lick_aligned_hits)
    print(lick_aligned_hits)
