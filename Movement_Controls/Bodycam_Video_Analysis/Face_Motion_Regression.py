...
def regress_face_motion(base_directory):
    ...
    face_motion = np.load(os.path.join(base_directory, "Mousecam_analysis", "Face_Motion_Temporal_Components.npy"))
    ...
    widefield_start = 20000
    widefield_stop = widefield_start + 5000

    selected_mousecam_frames = []
    for widefield_index in range(widefield_start, widefield_stop):
        mousecam_index = widefield_to_mousecam_frame_dict[widefield_index]
        selected_mousecam_frames.append(mousecam_index)

    # Get Delta F Snapshot
    delta_f_data_selection = delta_f_matrix[widefield_start:widefield_stop]
    mousecam_data_selection = face_motion[selected_mousecam_frames]

    # Create Model
    model = Ridge()
    model.fit(X=mousecam_data_selection, y=delta_f_data_selection)
    ...
    # Predicted Delta F
    predicited_data = model.predict(mousecam_data_selection)
...
base_directory = r"/media/matthew/External_Harddrive_1/Processed_Widefield_Data/NRXN78.1D/2020_12_07_Switching_Imaging"
