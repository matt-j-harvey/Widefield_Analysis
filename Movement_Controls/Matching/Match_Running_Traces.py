...
def match_trials(base_directory, visual_onsets_file, odour_onsets_file, trial_start_window, trial_stop_window, axis):
    ...
    # Get Mean Vectors
    visual_running_means = np.mean(visual_running_traces[:, 400:800], axis=1)
    odour_running_means = np.mean(odour_running_traces[:, 400:800], axis=1)
    ...
    visual_x_values = np.subtract(visual_x_values, 360)
    odour_x_values = np.subtract(odour_x_values, 360)
...
for base_directory in controls:
    ...
    np.save(os.path.join(base_directory, "Stimuli_Onsets", "Vis_2_Visual_Matched.npy"), matched_visual_onsets)
    np.save(os.path.join(base_directory, "Stimuli_Onsets", "Vis_2_Odour_Matched.npy"), matched_odour_onsets)
    count += 1
plt.show()
