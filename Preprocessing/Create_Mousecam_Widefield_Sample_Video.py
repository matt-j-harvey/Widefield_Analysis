...
def get_delta_f_sample_from_svd(base_directory, sample_start, sample_end, window_size=3):
    ...
    # Extract Raw Delta F
    transformed_data = np.load(os.path.join(base_directory, "SVD_Compressed_Delta_F.npy"))
    model_filepath = os.path.join(base_directory, "SVD_Model.sav")
    svd_model = pickle.load(open(model_filepath, 'rb'))
    delta_f_sample = transformed_data[sample_start:sample_end]
    delta_f_sample = svd_model.inverse_transform(delta_f_sample)
...
delta_f_directory = r"/media/matthew/External_Harddrive_3/MVAR_Data/Control_Data/NXAK22.1A/2021_10_14_Switching_Imaging"
