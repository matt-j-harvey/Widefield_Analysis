...
class masking_window(QWidget):
    ...
    def __init__(self, session_list, atlas_outline, atlas_regions, parent=None):
        ...
        for session_index in range(self.number_of_sessions):
            ...
            combined_retinotopy_image = np.load(os.path.join(self.session_list[session_index], "Combined_Sign_map_with_anatomy_full_size.npy"))
            print(np.shape(combined_retinotopy_image))

            # Add To List
            self.example_image_list.append(combined_retinotopy_image)

        # Get List Of Alignment Dictionaries
        self.variable_dictionary_list = []
        for session_index in range(self.number_of_sessions):

            variable_dictionary_directory = os.path.join(self.session_list[session_index], "Churchland_Atlas_Alignment_Dictionary_Full_Size.npy")
    ...
    def transform_atlas_regions(self, image, variable_dictionary):

        unique_values = list(set(np.unique(image)))
        transformed_mask = np.zeros((self.image_height, self.image_width))

        for value in unique_values:
            value_mask = np.where(image == value, 1, 0)
            value_mask = self.transform_mask_or_atlas(value_mask, variable_dictionary)
    ...
    def set_alignment(self):
        ...
        # Transform Atlas Regions
        transformed_atlas_regions = self.transform_atlas_regions(self.atlas_regions, self.atlas_variable_dictionary)
        np.save(os.path.join(session_list[self.current_session_index], "Atlas_Regions_Aligned_To_Sign_Map.npy"), transformed_atlas_regions)
        plt.imshow(transformed_atlas_regions)
        plt.show()
...
def align_sessions(session_list, atlas_outlines, atlas_regions):

    app = QApplication(sys.argv)
    window = masking_window(session_list, atlas_outlines, atlas_regions)
    window.show()
    app.exec_()
...
session_list = ["/media/matthew/External_Harddrive_2/Cortex_Wide_Opto/KPGC3.1D/2022_12_14_Retinotopy_Left"]
...
atlas_regions_location = r"/home/matthew/Documents/Allen_Atlas_Templates/churchland_atlas.npy"
atlas_regions = np.load(atlas_regions_location)

align_sessions(session_list, atlas_outline, atlas_regions)
