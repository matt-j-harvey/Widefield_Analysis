...
def create_behaviour_tensors(base_directory, data_root_directory, tensor_root_directory, onset_file_list, start_window, stop_window, start_cutoff):
    ...
    # Load Design Matrix
    design_matrix = np.load(os.path.join(full_base_directory, "Behaviour_Ridge_Regression", "Behaviour_Matrix.npy"))
...
def create_combined_design_matrix(base_directory, data_root_directory, tensor_root_directory, onset_file_list, start_window, stop_window):
    ...
    # Create Dictionaries
    stimuli_design_matrix_dict = create_stimuli_design_dictionary(onset_file_list, int(start_window), int(stop_window))
    behaviour_design_matrix_dict = np.load(os.path.join(data_root_directory, base_directory, "Behaviour_Ridge_Regression", "Behaviour_matrix_key_dict.npy"), allow_pickle=True)[()]
...
def create_full_model_design_matrix(base_directory, data_root_directory, tensor_root_directory, start_window, stop_window, onset_files, start_cutoff=3000, lick_lags=14, running_lags=14, n_face_components=360):

    # Check Tensor Directory
    check_tensor_directory(tensor_root_directory, base_directory)

    # Create Behaviour Design Matrix
    Create_Behaviour_Matrix.create_design_matrix(os.path.join(data_root_directory, base_directory),  lick_lags=lick_lags, running_lags=running_lags, n_face_components=n_face_components)
