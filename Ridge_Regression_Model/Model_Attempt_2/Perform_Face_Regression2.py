...
def regress_activity_onto_whole_video(base_directory, early_cutoff=3000):
    ...
    print("BOdycam Train", np.shape(bodycam_train))
    print("Lick Train", np.shape(lick_train))
    print("Running Train", np.shape(running_train))
    #train_x = np.hstack([bodycam_train, lick_train, running_train])
    #test_x = np.hstack([bodycam_test, lick_test, running_test])
    train_x = bodycam_train
    test_x = bodycam_test
    ...
    np.save(os.path.join(save_directory, "Face_Video_R2.npy"), r_squared)
    np.save(os.path.join(save_directory, "Face_Video_Coefs.npy"), regression_coefs)
    pickle.dump(model, open(os.path.join(save_directory, 'Face_Model.sav'), 'wb'))
    #np.save(os.path.join(save_directory, "Face_Coefs_Partial_Det.npy"), partial_determinant_matrix)
