...
def get_bodycam_data(base_directory):
    ...
    # Create Tables File
    bodycam_motion_energy_file = os.path.join(save_directory, "Bodycam_Data.h5")
    bodycam_motion_energy_container = tables.open_file(bodycam_motion_energy_file, "w")
    bodycam_motion_array = bodycam_motion_energy_container.create_earray(bodycam_motion_energy_container.root, 'Data', tables.UInt8Atom(), shape=(0, 480 * 640),  expectedrows=frameCount-1)
    ...
    for frame_index in tqdm(range(frameCount)):
        ...
        current_frame_data = np.ndarray.astype(current_frame_data, np.uint8)

        # Flatten
        current_frame_data = np.reshape(current_frame_data, 480 * 640)

        # Write To Tables File
        bodycam_motion_array.append([current_frame_data])
...
def get_bodycam_svd(base_directory):
    ...
    bodycam_motion_energy_file = os.path.join(save_directory, "Bodycam_Data.h5")
    bodycam_motion_energy_container = tables.open_file(bodycam_motion_energy_file, "r")
    bodycam_motion_array = bodycam_motion_energy_container.root["Data"]
    number_of_frames = np.shape(bodycam_motion_array)[0]
    ...
    for chunk_index in tqdm(range(number_of_chunks)):
        chunk_start = chunk_starts[chunk_index]
        chunk_stop = chunk_stops[chunk_index]
        chunk_data = bodycam_motion_array[chunk_start:chunk_stop]
        model.fit(chunk_data)
    ...
    # Save Model
    np.save(os.path.join(save_directory, "Bodycam_Transformed_Data.npy"), transformed_data)
    np.save(os.path.join(save_directory, "Bodycam_Components.npy"), components)

    # Save Model
    model_file = os.path.join(base_directory, "Mousecam_Analysis",  "Bodycam_No_Motion_Model.sav")
    pickle.dump(model, open(model_file, 'wb'))
...
#get_bodycam_data(base_directory)
get_bodycam_svd(base_directory)

#get_bodycam_motion_energy(base_directory)
#view_bodycam_motion_energy(base_directory)
#compute_body_motion_svd(base_directory)
#extract_face_motion_only(base_directory)
#visualise_svd_decomposition(base_directory)
#extract_face_motion_only(base_directory)
#create_face_motion_video(base_directory)#
#get_face_svd(base_directory)