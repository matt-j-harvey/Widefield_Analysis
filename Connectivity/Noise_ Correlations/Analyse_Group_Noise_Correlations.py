...
def analyse_group_matrix(mouse_list, group_name, signal_or_noise, tensor_save_directory, mean_matrix_save_directory, cmap):
    ...
    np.save(os.path.join(mean_matrix_save_directory, "Mean_" + group_name + "_" + signal_or_noise + "_Modulation.npy"), mean_modulation_matrix)
    np.save(os.path.join(mean_matrix_save_directory, "Mean_" + group_name + "_" + signal_or_noise + "_Modulation_Significant.npy"), thresholded_mean_modulation_matrix)

    plt.title(group_name + "_" + signal_or_noise + "_t_stats")
    t_stat_mangnitude = 4
    plt.imshow(t_stats, cmap=cmap, vmin=-t_stat_mangnitude, vmax=t_stat_mangnitude)
    plt.show()

    plt.title(group_name + "_" + signal_or_noise + "Mean_Modulation")
    plt.imshow(mean_modulation_matrix, cmap=cmap, vmin=-0.5, vmax=0.5)
    plt.show()

    plt.title(group_name + "_" + signal_or_noise + "_Thresholded_Mean")
...
def compare_genotype_modulation(control_mouse_list, mutant_mouse_list, tensor_save_directory, tensor_names, signal_or_noise):
    ...
    modulation_map = Noise_Correlation_Utils.create_image_from_data(mean_modulation_vector, indicies, image_height, image_width)
...
# Get Colourmap
cmap = Noise_Correlation_Utils.get_musall_cmap()
