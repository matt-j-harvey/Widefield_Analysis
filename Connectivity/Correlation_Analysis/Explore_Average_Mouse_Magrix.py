...
class correlation_explorer(QWidget):
    ...
    def __init__(self, correlation_maps, index_map, parent=None):
        ...
        for mouse in self.correlation_maps:
            ...
            self.display_widget_list.append(display_widget)
        ...
        for mouse_index in range(len(self.display_widget_list)):
            self.layout.addWidget(self.display_widget_list[mouse_index], 0, mouse_index, 1, 1)
    ...
    def getPixel(self, pos, imageview):
        ...
        for mouse_index in range(len(self.display_widget_list)):
            ...
            # Get Correlation Map
            modulation_image = self.correlation_maps[mouse_index][pixel_index]

            # Set Display View Image
            display_view = self.display_view_list[mouse_index]
...
def load_correlation_maps_and_index_maps(mouse_list, tensor_save_directory, tensor_names, correlation_matrix_filename):
    ...
    for mouse in tqdm(mouse_list):
        ...
        for base_directory in mouse:
            ...
            # Add To List
            mouse_correlation_matrix_list.append(modulation_matrix)

        # Get Mean Correlation Matrix
        mouse_correlation_matrix_list = np.array(mouse_correlation_matrix_list)
        mouse_correlation_matrix = np.mean(mouse_correlation_matrix_list, axis=0)
        ...
        modulation_maps = construct_correlation_maps(mouse_correlation_matrix, indicies, image_height, image_width)

        # Add To List
        correlation_map_list.append(modulation_maps)
    ...
    return correlation_map_list, index_map
...
def explore_multiple_correlation_matricies(mouse_list, tensor_names, correlation_matrix_filename):
    ...
    correlation_maps, index_map = load_correlation_maps_and_index_maps(mouse_list, tensor_save_directory, tensor_names, correlation_matrix_filename)
    print("Showing Maps")
    window = correlation_explorer(correlation_maps, index_map)
