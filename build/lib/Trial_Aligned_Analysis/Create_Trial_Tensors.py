...
def create_trial_tensor(base_directory, onsets_file, start_window, stop_window, tensor_save_directory,
    ...
    if ridge_regression_correct == True:
        ...
        if extended_tensor == True:
            ...
            regression_dict = np.load(os.path.join(base_directory, "Ridge_Regression", "Regression_Dictionary_Simple.npy"), allow_pickle=True)[()]
            design_matrix = np.load(os.path.join(base_directory, "Ridge_Regression", "Design_Matrix.npy"))
