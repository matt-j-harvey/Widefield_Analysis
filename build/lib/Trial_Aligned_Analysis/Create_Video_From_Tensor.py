...
def get_atlas_outline_pixels():
    ...
    # Transform Atlas
    atlas_outline = widefield_utils.transform_mask_or_atlas(atlas_outline, transformation_dict)
...
def create_activity_video(mean_activity_tensors, trial_start, plot_titles, save_directory, indicies, image_height, image_width, timestep=36):
    ...
    background_indicies = widefield_utils.get_background_pixels(indicies, image_height, image_width)
    ...
    number_of_columns = number_of_conditions
    number_of_rows = 1
    ...
    # Plot Each Timepoint
    window_size = 3
    for timepoint in tqdm(range(window_size, number_of_timepoints)):

        time_text = str(timepoint_list[timepoint]-1800) + "ms"
        x_pos = 30
        y_pos = 75
        ...
        for condition_index in range(number_of_conditions):
            ...
            # Select Images
            condition_activity_image = mean_activity_tensors[condition_index][timepoint-window_size:timepoint]
            condition_activity_image = np.mean(condition_activity_image, axis=0)
            condition_activity_image = widefield_utils.create_image_from_data(condition_activity_image, indicies, image_height, image_width)
        ...
        plt.savefig(os.path.join(save_directory, str(timepoint_count).zfill(3) + ".svg"))
        plt.clf()
