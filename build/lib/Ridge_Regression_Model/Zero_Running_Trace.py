...
class running_threshold_window(QWidget):

    def __init__(self, session_directory_list, parent=None):
        super(running_threshold_window, self).__init__(parent)

        # Setup Window
        self.setWindowTitle("Set Running Baseline")
        self.setGeometry(0, 0, 1500, 500)

        # Create Variable Holders
        self.session_directory_list = session_directory_list
        self.channel_dictionary = widefield_utils.create_stimuli_dictionary()
        self.default_lick_threshold = 0.43
        self.current_lick_threshold = 0.43
        ...
        self.set_lick_threshold_button = QPushButton("Set Running Baseline")
    ...
    def load_all_data(self):
        ...
        print("Loading Running Data, Please Wait: ")
        ...
        for session_index in tqdm(range(number_of_sessions)):
            ...
            # Load Lick Trace
            ai_data = widefield_utils.load_ai_recorder_file(current_session)
            lick_trace = ai_data[self.channel_dictionary["Running"]]
            self.lick_trace_list.append(lick_trace)

            # See If We Already Have a Lick Threshold Set
            if os.path.exists(os.path.join(current_session, "Running_Baseline.npy")):
                lick_threshold = np.load(os.path.join(current_session, "Running_Baseline.npy"))
    ...
    def set_lick_threshold(self):
         ...
         # Get Output Path
        file_save_directory = os.path.join(self.session_directory_list[self.current_session_index], "Running_Baseline.npy")
...
"""
def zero_running_traces(session_directory):
...
zero_running_traces(session_list)
"""