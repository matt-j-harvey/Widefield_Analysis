...
def get_group_averages(control_session_list, mutant_session_list, condition_list, start_window, stop_window, selected_region, min_trials=5):

    # Get Number Of Conditions
    number_of_conditions = len(condition_list)

    # Create Figure
    figure_1 = plt.figure()
    gridspec_1 = GridSpec(nrows=4, ncols=number_of_conditions, figure=figure_1)
    ...
    for condition_index in range(number_of_conditions):
        condition = condition_list[condition_index]
        
        control_neural_traces, control_running_traces, control_lick_traces = get_condition_average_traces(control_session_list, condition, min_trials)
        mutant_neural_traces, mutant_running_traces, mutant_lick_traces = get_condition_average_traces(mutant_session_list, condition, min_trials)
        ...
        # Create Axes
        all_sessions_neural_axis = figure_1.add_subplot(gridspec_1[0, condition_index])
        neural_axis = figure_1.add_subplot(gridspec_1[1, condition_index])
        running_axis = figure_1.add_subplot(gridspec_1[2, condition_index])
        lick_axis = figure_1.add_subplot(gridspec_1[3, condition_index])
...
pixel_borders = np.load("/media/matthew/Expansion/Widefield_Analysis/Consensus_Clustering/cluster_outlines.npy")
border_mask = np.where(pixel_borders > 0, 1, 0)
border_pixels = np.nonzero(border_mask)
print(border_pixels)
...
"""
condition_0 = "Stimuli_Onsets_Matched_Reaction_Times_900_1000.npy"
...
condition_3 = "Stimuli_Onsets_Matched_Reaction_Times_1200_1300.npy"
condition_4 = "Stimuli_Onsets_Matched_Reaction_Times_1300_1400.npy"



pixel_mask = np.where(pixel_assignments == selected_region, 1, 0)
pixel_mask[border_pixels] = 1
plt.imshow(pixel_mask, cmap='Greys')
plt.show()


condition_list = [condition_0, condition_1, condition_2, condition_3, condition_4]
get_group_averages(control_session_list, mutant_session_list, condition_list, start_window, stop_window, selected_region=selected_region, min_trials=5)


condition_1 = "visual_2_correct_onsets.npy"
condition_2 = "visual_2_incorrect_onsets.npy"
condition_list = [condition_1, condition_2]
#get_group_averages(control_session_list_post, mutant_session_list_post, condition_list, start_window, stop_window, selected_region=17, min_trials=5)
get_group_averages(control_session_list, mutant_session_list, condition_list, start_window, stop_window, selected_region=selected_region, min_trials=5)
"""