...
def get_group_averages(control_session_list, mutant_session_list, condition_1, condition_2, start_window, stop_window, selected_region):
    ...
    for base_directory in control_session_list:
        ...
        # Get Average Traces
        result = get_session_average_traces(base_directory, condition_1, condition_2, start_window, stop_window)

        if result != None:
            condition_1_mean  = result[0]
            condition_1_sd  = result[1]
            condition_2_mean  = result[2]
            condition_2_sd = result[3]
    ...
    control_all_mice_axis = figure_1.add_subplot(gridspec_1[0, 0])
    mutant_all_mice_axis = figure_1.add_subplot(gridspec_1[0, 1])
    control_group_average_axis = figure_1.add_subplot(gridspec_1[1, 0])
    mutant_group_average_axis = figure_1.add_subplot(gridspec_1[1, 1])

    # Plot Group Averages
    x_values = list(range(start_window, stop_window))
    x_values = np.multiply(x_values, 36)

    control_group_average_axis.plot(x_values, control_condition_1_group_average, c='b', linestyle='--')
    control_group_average_axis.plot(x_values, control_condition_2_group_average, c='b')
    control_group_average_axis.fill_between(x=x_values, y1=control_condition_1_lower_limit, y2=control_condition_1_upper_limit, color='b', alpha=0.2)
    control_group_average_axis.fill_between(x=x_values, y1=control_condition_2_lower_limit, y2=control_condition_2_upper_limit, color='b', alpha=0.2)

    mutant_group_average_axis.plot(x_values, mutant_condition_1_group_average, c='g', linestyle='--')
    mutant_group_average_axis.fill_between(x=x_values, y1=mutant_condition_1_lower_limit, y2=mutant_condition_1_upper_limit, color='g', alpha=0.2)
    mutant_group_average_axis.plot(x_values, mutant_condition_2_group_average, c='g')
    mutant_group_average_axis.fill_between(x=x_values, y1=mutant_condition_2_lower_limit, y2=mutant_condition_2_upper_limit, color='g', alpha=0.2)


    # Plot Individual Mice
    for mouse in condition_1_control_averages:
        control_all_mice_axis.plot(x_values, mouse, c='b', linestyle='--')
    for mouse in condition_2_control_averages:
        control_all_mice_axis.plot(x_values, mouse, c='b')

    for mouse in condition_1_mutant_averages:
        mutant_all_mice_axis.plot(x_values, mouse, c='g', linestyle='--')
    for mouse in condition_2_mutant_averages:
        mutant_all_mice_axis.plot(x_values, mouse, c='g')
