...
def perform_decoding(delta_f_matrix, condition_1_onsets, condition_2_onsets, start_window, stop_window):
    ...
    for timepoint_index in range(number_of_timepoints):
        ...
        #print("Timepoint Data", np.shape(timepoint_data))
    ...
    print("Mean Weights List", np.shape(mean_weights_list))
    #mean_weights_list = np.reshape(mean_weights_list, (np.shape(mean_weights_list)[0], np.shape(mean_weights_list)[2]))


    return temporal_score_list, temporal_sd_list
...
def run_decoding_analysis(session_list, condition_1, condition_2, start_window, stop_window):
    ...
    for session_index in range(number_of_sessions):
        ...
        decoding_scores, decoding_sds = perform_decoding(activity_matrix, vis_1_onsets, vis_2_onsets, start_window, stop_window)
        ...
        plt.plot(decoding_scores, c=plot_colour)
...
session_list = [r"/media/matthew/Seagate Expansion Drive2/No_Filter_Test"]
