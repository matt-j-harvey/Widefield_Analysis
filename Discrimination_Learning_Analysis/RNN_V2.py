...
def train_network(rnn, input_tensor_list, hidden_state_list, activity_tensor_list, crtierion, optimiser, number_of_trials, number_of_timepoints, number_of_regions):
...
def get_validation_loss(rnn, input_tensor_list, hidden_state_list, activity_tensor_list, number_of_trials, number_of_timepoints, number_of_regions, crtierion):
    loss_list = []
    for trial in range(number_of_trials):
...
for i in range(n_iters):
    ...
    if (i) % print_steps == 0:
        ...
        validation_loss = get_validation_loss(rnn, test_combined_inputs, test_combined_initial_states, test_combined_activity, number_of_test_samples, trial_length, number_of_regions, criterion)
        all_validation_losses.append(validation_loss)

        print("Iteration: ", i, "Training loss:", training_loss.detach().numpy(), "Validation loss:", validation_loss)
