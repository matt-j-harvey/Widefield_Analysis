...
def get_trial_tensor(delta_f_matrix, onset_list, start_window, stop_window, perform_smoothing=False, base_directory=None):
    ...
    for onset in onset_list:
        ...
        #print("Trial Start:", trial_start, "Trial Stop:", trial_stop, "Trial Data Shape", np.shape(trial_data))
...
def load_neural_data(delta_f_matrix, condition_onsets_list, start_window, stop_window, base_directory):
...
def create_design_matrix(number_of_conditions, trial_length, condition_trials, running_tensor, lick_tensor, neural_tensor):
    ...
    print("Behaviour Design Matrix", np.shape(behavioural_design_matrix))
...
def remove_early_onsets(onsets, number_of_timepoints, stop_window, window=3000):

    thresholded_onsets = []

    for onset in onsets:
        if onset > window and (onset + stop_window) < number_of_timepoints:
...
def perform_regression_analysis(session_list, start_window, stop_window, condition_onset_files, model_name, min_trials=2, max_trials=150):
    ...
    for session_index in range(len(session_list)):
        ...
        print("Session: ", session_index, " of ", len(session_list), " at ", datetime.now(), base_directory)
        ...
        print("Number of timepoints", number_of_timepoints, "Number of Pixels", number_of_pixels)
        ...
        # Load Onsets
        onsets_for_all_conditions = True
        ...
        for onset_file in condition_onset_files:
            ...
            #print("Condition Onsets", condition_onsets)

            condition_onsets = remove_early_onsets(condition_onsets, number_of_timepoints, stop_window)
            print("Condition Onsets", len(condition_onsets))

            if len(condition_onsets) > max_trials:
                condition_onsets = condition_onsets[0:max_trials]

            if len(condition_onsets) < min_trials:
                onsets_for_all_conditions = False
    
            condition_onsets_list.append(condition_onsets)

        if onsets_for_all_conditions:
            ...
            neural_tensor = load_neural_data(delta_f_matrix, condition_onsets_list, start_window, stop_window, base_directory)
