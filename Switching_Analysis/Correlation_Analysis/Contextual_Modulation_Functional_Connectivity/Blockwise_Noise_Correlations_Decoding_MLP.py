...
def load_onsets_blockwise(base_directory, onset_file, onset_behaviour_matrix_field, min_block_size=8, early_onset_cutoff = 3000):
...
def perform_k_fold_cross_validation(data, labels, number_of_folds=5):
    ...
    for train_index, test_index in k_fold_object.split(data, y=labels):
        ...
        #model = MLPClassifier(activation='logistic', max_iter=5000)
...
def decode_context_connectivity_matricies(condition_1_matrix_list, condition_2_matrix_list):
    ...
    mean_score = perform_k_fold_cross_validation(combined_data, combined_labels)
...
def run_decoding_analysis(session_list,condition_1, condition_2, start_window, stop_window, subtract_baseline):
    ...
    mean_score = decode_context_connectivity_matricies(all_condition_1_matricies, all_condition_2_matricies)
...
meta_mean_score_list = []
for x in range(10):
    mean_score = run_decoding_analysis(session_list, condition_1, condition_2, start_window, stop_window, subtract_baseline)
    print("Mean score", mean_score)
    meta_mean_score_list.append(mean_score)

print("Grand MEan", np.mean(meta_mean_score_list))
